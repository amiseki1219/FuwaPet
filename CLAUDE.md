...
Main → Gacha（ガチャ）
Main → RoomEdit（もようがえ）
Main → SNS（ひろば）※v1.0で自由投稿を実装（モデレーション/通報/ブロック込み）。フレンド間のやりとり（DM・訪問・タイムライン絡み等）はv1.1
Care → Main（戻る）
```



#### フレンド機能のスコープ（v1.0 / v1.1）

- **v1.0**: 「繋がる」までを実装 ＝ フレンド申請・承認・フレンド一覧・公開プロフィール。
- **v1.1**: フレンド"間"のやりとり（DM・訪問・タイムラインでの相互作用）は v1.1 送り。
- **必須付随項目（v1.0）**: 自由投稿SNS＋公開プロフィールに伴うモデレーション・通報・ブロックは v1.0 の必須付随項目（詳細設計は 8/25 のバックエンド作戦会議で確定）。
