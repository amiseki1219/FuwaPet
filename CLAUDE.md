...
下部ナビ（5つ）: コレクション・もようがえ（RoomEdit）・ショップ・**ガチャ**・クエスト
（お知らせは右側に配置。パネルを開くだけでシーン遷移しないためナビには入れない）
```

※ガチャは **2026/8/21 に廃止**（Lv解放型ショップに置き換え）。ナビの `GatyaBtn` は
　ひろば廃止で空いた枠へ移設する。移設時は `ButtonGroup` の GridLayoutGroup の
　CellSize.x を 170→140 にすること（140×6+10×5=890px で現状と同じ全幅に収まる）  
...
Main → Gacha（ガチャ）
Main → RoomEdit（もようがえ）
（Main → SNS（ひろば）は 2026/8/21 に廃止。Scene・ボタン・遷移コードも削除する）
Care → Main（戻る）
```
...
| 2026/8/21 | 【大規模改訂】ひろば(SNS)機能を廃止（掲示板・フレンド・公開プロフィール・モデレーション/通報/ブロックを全てスコープ外へ。§13・付録F の矛盾も解消）・信頼度ランキングは作らないと決定・ガチャを廃止し信頼度Lv解放型ショップへ転換（対象=家具/きせかえ/フレーム、解放段階=Lv.40まで10刻み・以降5刻み、Lv.75以降はアプデ）・Lv.100特典「等身大ぬいぐるみ先着500体」を廃止（景品表示法の景品規制・物理コスト・数年後の履行義務・競争要素が世界観と衝突）・Lv.100をカンストとしない方針・下部ナビを7つ→5つに変更しひろばの枠へガチャを移設（CellSize.x 170→140）・サブスクを3プラン→2プランに全面改訂（プランA ¥500 / プランB ¥2,000、v1.1で中間に3段目追加。主軸=会話回数+信頼度倍率、♡付与は廃止、🪙付与+30/+100、広告非表示はプランAから、感情レポートは無料=節目のみ/A=月2/B=週1）・信頼度倍率 ×1.0/×1.2/×1.5 を新設・チケット制を「週間お話し上限(35/105/350回・繰越なし・加入日起点・サーバー管理必須)」と「所持チケット(繰越あり・失効なし)」に分離・入力文字数制限200文字・出力トークン上限150〜200・会話AIをGeminiからOpenAIへ全面置換（技術スタック・§16・Q&A・APIキー欄）・1日の最大ptを153pt→132ptに訂正（実質100pt前後で設計）・パズル仕様を実装に合わせて訂正（1日5プレイ・失敗も消費・進捗は翌日リセット） |
