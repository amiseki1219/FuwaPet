...
namespace Yurufu.Bath.Foam
{
    ...
    ///   モデルを触らずに左右を分離する。
    ///   ★この割り当ては BathFoamShell.shader の MaskUV() と一致させること。
    /// </summary>
    public class BathFoamMask
    {
        ...
        public BathFoamMask(string label, int w, int h, Material brushMaterial)
        {
            _label = label;
        ...
        private RenderTexture Create(int w, int h, string name)
        {
            ...
            if (rt == null || !rt.Create())
            {
                ...
                rt.Create();
                Debug.LogWarning($"[BathFoam] {name}: R8_UNorm が使えないため ARGB32 で作成しました");
            }

            rt.name       = "~FoamMask_" + name;
            rt.filterMode = FilterMode.Bilinear;
            rt.wrapMode   = TextureWrapMode.Clamp;
            // ★hideFlags は付けない。Dispose() で明示的に Release + Destroy する
            return rt;
        }
        ...
        /// </summary>
        public void PaintSegment(Vector2 uvFrom, Vector2 uvTo, bool upperHalf, BathFoamConfig cfg)
        {
        ...
        public void Dispose()
        {
            if (_read  != null) { _read.Release();  Object.Destroy(_read);  _read  = null; }
            if (_write != null) { _write.Release(); Object.Destroy(_write); _write = null; }
        }
    }
}
