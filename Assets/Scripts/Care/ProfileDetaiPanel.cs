...
public class ProfileDetailPanel : MonoBehaviour
{
    ...
    public void UpdateDisplay()
    {
        ...
        // --- 4. 課金バッジ ---
        if (badgeImage != null && BadgeManager.Instance != null)
        {
            string bestBadgeId = BadgeManager.Instance.GetCurrentBestBadgeId();
            if (!string.IsNullOrEmpty(bestBadgeId))
            {
                Sprite badgeSprite = Resources.Load<Sprite>("BadgeUI/" + bestBadgeId);
                if (badgeSprite != null) { badgeImage.enabled = true; badgeImage.texture = badgeSprite.texture; }
